import { subtask, task, extendConfig, extendEnvironment } from 'hardhat/config';
import { SolangCompiler, deriveKeyPairFromEvmPrivateKey, deriveKeyPairFromMnemonic, TVASigner, ContractDeployer, TVAContract } from '@tva-protocol/sdk';
...
async function loadArtifact(name, hre) {
  ...
  const artifactsDir = path2.join(
    hre.config.paths.root,
    hre.config.tva.artifactsDir
  );
  const contractDir = path2.join(artifactsDir, name);
  const artifactPath = path2.join(contractDir, `${name}.json`);
  const wasmPath = path2.join(contractDir, `${name}.wasm`);
  try {
    await fs2.access(artifactPath);
  } catch {
    throw new Error(
      `Contract artifact not found for "${name}". Make sure the contract has been compiled with "npx hardhat tva:compile".`
    );
  }
  const artifactContent = await fs2.readFile(artifactPath, "utf-8");
  const artifact = JSON.parse(artifactContent);
  const wasm = await fs2.readFile(wasmPath);
...
subtask(SUBTASK_TVA_COMPILE_GET_SOURCES).setDescription("Gets all Solidity source files for TVA compilation").setAction(async (_, hre) => {
...
subtask(SUBTASK_TVA_COMPILE_SOLANG).setDescription("Compiles Solidity sources using Solang for Soroban").addParam("sources", "Array of source file paths").setAction(async ({ sources }, hre) => {
  ...
  const compiler = new SolangCompiler({
    ...
    importPaths: [
      ...
      path2.join(hre.config.paths.root, "node_modules"),
      ...config.importPaths
    ]
  });
  ...
  for (const sourcePath of sources) {
    try {
      console.log(`  Compiling ${path2.basename(sourcePath)}...`);
      ...
      console.error(`  Error compiling ${path2.basename(sourcePath)}: ${message}`);
    }
  }
  if (errors.length > 0) {
    console.error(`
...
subtask(SUBTASK_TVA_COMPILE_GENERATE_ARTIFACTS).setDescription("Generates Hardhat-compatible artifacts from compiled contracts").addParam("contracts", "Array of compiled contracts").setAction(async ({ contracts }, hre) => {
  const artifactsDir = path2.join(
    hre.config.paths.root,
    hre.config.tva.artifactsDir
  );
  await fs2.mkdir(artifactsDir, { recursive: true });
  const artifacts = [];
  for (const contract of contracts) {
    const contractDir = path2.join(artifactsDir, contract.name);
    await fs2.mkdir(contractDir, { recursive: true });
    const wasmPath = path2.join(contractDir, `${contract.name}.wasm`);
    await fs2.writeFile(wasmPath, contract.wasm);
    const abiPath = path2.join(contractDir, `${contract.name}.json`);
    const abiEntries = [
    ...
    await fs2.writeFile(abiPath, JSON.stringify(artifact, null, 2));
    artifacts.push({
      name: contract.name,
...
task(TASK_TVA_COMPILE, "Compiles Solidity contracts for TVA/Soroban using Solang").addFlag("force", "Force recompilation of all contracts").addFlag("quiet", "Suppress compilation output").setAction(async ({ force: _force, quiet }, hre) => {
  if (!quiet) {
    console.log("\n========================================");
...
task("compile", "Compiles the entire project").setAction(async (args, hre, runSuper) => {
  await runSuper(args);
...
subtask(SUBTASK_TVA_GET_SIGNERS).setDescription("Gets TVA signers from network configuration").setAction(async (_, hre) => {
  ...
  if (Array.isArray(accounts)) {
    for (const account of accounts) {
      const keyPair = account.startsWith("0x") ? deriveKeyPairFromEvmPrivateKey(account) : await deriveKeyPairFromMnemonic(account);
...
subtask(SUBTASK_TVA_GET_CONTRACT_FACTORY).setDescription("Gets a contract factory for deployment").addParam("name", "Contract name").setAction(async ({ name }, hre) => {
  const artifactsDir = path2.join(
    hre.config.paths.root,
    hre.config.tva.artifactsDir
  );
...
task(TASK_TVA_DEPLOY, "Deploys a contract to TVA/Soroban").addPositionalParam("contract", "Contract name to deploy").addOptionalVariadicPositionalParam("args", "Constructor arguments", []).setAction(async ({ contract, args }, hre) => {
  console.log("\n========================================");
...
export { createSampleConfig, tvaNetworks };
